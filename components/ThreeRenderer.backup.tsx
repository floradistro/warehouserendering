...
function RoomLabels({ floorplan }: { floorplan: FloorplanData }) {
  ...
  // Handle North Dry Room - check for the single dry room
  ...
  if (northDryRoomWalls.length > 0) {
    // We have the north dry room - label it as Dry Room
    
    // Dry Room (North dry room) - between y=211.5209 and y=222
    const dryCenterY = 211.5209 + (10.4791 / 2) // Center of the 10.4791' wide room
    const dryCenterX = 76.875 + (33.5 / 2) // Center of the 33.5' long room
    roomPositions.push({ roomNumber: 'Dry Room', centerY: dryCenterY, centerX: dryCenterX })
    ...
    if (leftWall) {
      // Dry Room (west side) - between left wall and first divider
      const dryCenterX = (leftWall.position.x + leftWall.dimensions.width/2 + dividerX) / 2
      roomPositions.push({ roomNumber: 'Dry Room', centerY: room1CenterY, centerX: dryCenterX })
...
function Scene({ onCameraReady, snapPointsCache, showFraming, showDrywall, cameraTarget, lockedTarget }: { 
  ...
  return (
    <>
      ...
      <RoomFloor
        position={[70.40625, -0.05, 36.83335]} // Center position
        width={80.6875}
      ...
      {/* East Longway Hallway: x = 107.75 to x = 112.75, y = 48.6667 to y = 198.0417 */}
      <RoomFloor
        position={[110.25, -0.05, 123.35415]} // Center position: x=(107.75+112.75)/2=110.25, y=(48.6667+198.0417)/2=123.35415
        width={5.0} // 5' hallway width
      ...
      {/* Area In Front of Dry Rooms West Side: x = 76.875 to x = 81.875, y = 198.0417 to y = 222 */}
      <RoomFloor
        position={[79.3125, -0.05, 210.02085]} // Center position: x=(76.875+81.875)/2=79.3125, y=(198.0417+222)/2=210.02085
        width={5.0} // Gap width between control area and dry rooms
        height={23.9583} // From Room 2 north wall to north wall (222 - 198.0417 = 23.9583)
        material="black-gloss-epoxy"
      />
      
      {/* East Hallway Behind Dry Rooms: x = 107.75 to x = 112.75, y = 198.0417 to y = 222 */}
      <RoomFloor
        position={[110.25, -0.05, 210.02085]} // Center position: x=(107.75+112.75)/2=110.25, y=(198.0417+222)/2=210.02085
        width={5.0} // 5' hallway width
        height={23.9583} // From south dry room to north exterior wall (222 - 198.0417 = 23.9583)
        material="black-gloss-epoxy"
      />
